package io.github.senhw.crm.config;
...
@Configuration
@EnableWebSecurity
public class CrmAppSecurityConfig extends WebSecurityConfigurerAdapter {
	...
	protected void configure(AuthenticationManagerBuilder auth) throws Exception {
		...
		auth.inMemoryAuthentication()
			.withUser(users.username("jack").password("pass123").roles("EMPLOYEE"))
			.withUser(users.username("maria").password("pass123").roles("EMPLOYEE", "MANAGER"))
			.withUser(users.username("sarah").password("pass123").roles("EMPLOYEE", "ADMIN"));
	}
